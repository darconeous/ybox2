{{
        ...
        This is a simple HTTP interface to X10.
}}
CON
...
pub httpServer | char, i, contentLength,authorized,queryPtr,house,unit,code
  repeat
    ...
    if strcomp(@httpMethod,string("GET"))
      ...
      elseif strcomp(@httpPath,string("/reboot"))
        ...
        socket.str(@HTTP_200)
        socket.str(@HTTP_CONNECTION_CLOSE)
        socket.txmimeheader(string("Refresh"),string("8;url=/"))        
        socket.str(@CR_LF)
        if strcomp(queryPtr,string("bootloader")) AND settings.findKey(settings#MISC_AUTOBOOT)
          socket.str(string("DISABLING AUTOBOOT",13,10))
          settings.revert
...
pri indexPage | i, tempF, tempC, p
  ...
  socket.str(string("<br/>"))
  httpOutputLink(string("/send?unit=12&code=7"),string("red button"),string("Unit 1 Off"))
  socket.str(string("<br/>"))
  httpOutputLink(string("/send?code=1"),string("red button"),string("All Off"))
  socket.str(string("<br/>"))
